...
export const Potrace = (function() {
  ...
  // Added helper to load directly from an existing canvas (Performance)
  function loadFromCanvas(sourceCanvas) {
    if (info.isReady) {
        clear();
    }
    imgCanvas.width = sourceCanvas.width;
    imgCanvas.height = sourceCanvas.height;
    var ctx = imgCanvas.getContext('2d');
    ctx.drawImage(sourceCanvas, 0, 0);
    loadBm();
    // Since we bypass imgElement.onload, we manually check callback
    // But process() handles the wait loop. loadBm sets isReady=true.
  }
  ...
  // Custom: Load directly from canvas to avoid async image loading
  function loadFromCanvas(sourceCanvas) {
    if (info.isReady) {
        clear();
    }
  ...
  // Custom: Expose raw paths for embroidery physics engine
  function getPaths() {
      return pathlist;
  }
  ...
  return{
    ...
    loadFromCanvas: loadFromCanvas, // Added
    getPaths: getPaths, // Added
